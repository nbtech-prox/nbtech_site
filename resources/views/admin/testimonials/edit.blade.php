...
@section('title', 'Editar testemunho | NBTech Admin')
@section('heading', 'Editar testemunho')

@section('content')
    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" class="panel p-6">
        @csrf
        @method('PUT')
        @include('admin.testimonials._form', ['testimonial' => $testimonial])
        <div class="mt-6 flex gap-3">
            <button class="btn-primary" type="submit">Atualizar</button>
            <a href="{{ route('admin.testimonials.index') }}" class="btn-secondary">Voltar</a>
        </div>
    </form>

    <form method="POST" action="{{ route('admin.testimonials.destroy', $testimonial) }}" class="mt-4" onsubmit="return confirm('Eliminar testemunho?')">
        @csrf
        @method('DELETE')

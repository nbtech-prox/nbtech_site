...
<div class="grid gap-4 md:grid-cols-3">
    <div>
        ...
        <input id="title" name="title" required value="{{ old('title', $quote?->title) }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
    </div>
    <div>
        <label class="mb-1 block text-sm font-medium" for="status">Estado</label>
        <select id="status" name="status" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
    ...
    <div>
        ...
        <select id="document_type" name="document_type" x-model="documentType" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
...
<div class="mt-4 grid grid-cols-1 gap-4" :class="documentType === 'proforma' ? 'md:grid-cols-2' : 'md:grid-cols-1'">
    ...
    <div x-show="documentType === 'proforma'" x-transition.opacity>
        ...
        <div class="relative">
            <input id="due_date" type="text" inputmode="numeric" autocomplete="off" placeholder="dd/mm/aaaa" name="due_date" :required="documentType === 'proforma'" x-model="dueDate" @click="openPicker('due')" @blur="normalizeDisplayDate('due')" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 pr-10 text-sm dark:border-slate-700 dark:bg-slate-900">
...
<div class="mt-4 grid gap-4 md:grid-cols-2">
    <div>
        ...
        <input id="client_name" name="client_name" required value="{{ old('client_name', $quote?->client_name) }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
    </div>
    <div>
        <label class="mb-1 block text-sm font-medium" for="client_email">Email</label>
        <input id="client_email" type="email" name="client_email" value="{{ old('client_email', $quote?->client_email) }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
    </div>
</div>
...
<div class="mt-4 grid gap-4 md:grid-cols-3">
    <div>
        ...
        <input id="client_phone" name="client_phone" value="{{ old('client_phone', $quote?->client_phone) }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
    </div>
    <div>
        <label class="mb-1 block text-sm font-medium" for="client_nif">NIF</label>
        <input id="client_nif" name="client_nif" value="{{ old('client_nif', $quote?->client_nif) }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
    </div>
    <div>
        <label class="mb-1 block text-sm font-medium" for="tax_rate">IVA (%)</label>
        <div class="number-stepper">
            <input id="tax_rate" type="number" step="0.01" min="0" max="100" name="tax_rate" value="{{ old('tax_rate', $quote?->tax_rate ?? '23.00') }}" x-model="taxRate" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
...
<div class="mt-4">
    ...
    <input id="client_address" name="client_address" value="{{ old('client_address', $quote?->client_address) }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">
</div>
...
<div class="mt-4">
    ...
    <textarea id="notes" name="notes" rows="4" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900">{{ old('notes', $quote?->notes) }}</textarea>
</div>
